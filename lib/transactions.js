...
exports.createGeneration = function(rpcData, blockReward, feeReward, recipients, poolAddress){
    ...
    for (var i = 0; i < recipients.length; i++) {
       var hash = bitcoin.address.fromBase58Check(recipients[i].address).hash;
       var address = bitcoin.address.fromBase58Check(recipients[i].address);
       console.log("Reward:", address, "-", hash);
       tx.addOutput(
           scriptFoundersCompile(hash),
